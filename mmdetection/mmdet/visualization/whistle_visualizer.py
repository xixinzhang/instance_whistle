...
class WhistleVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'masks' in instances:
            ...
            # base_colors = [np.array(cmap(i)[:3]) for i in range(len(labels))]
            # colors = [((c + np.random.uniform(-0.1, 0.1, 3)).clip(0, 1) * 255).astype(np.uint8)for c in base_colors]
            colors = high_contrast_colors(len(labels))
            ...
            masks = self._dilate_masks(masks)
            ...
            #             positions.append(centroids[largest_id]+ np.array([0, 15]))
    ...
    def _dilate_masks(self,
                      masks: np.ndarray,
                      kernel_size: int = 3,
                      iterations: int = 1) -> np.ndarray:
        """Dilate boolean masks to improve visibility when rendering."""
        if masks.size == 0:
            return masks
        ...
        dilated = [
            cv2.dilate(mask.astype(np.uint8), kernel, iterations=iterations)
            for mask in masks
        ]
        return np.stack(dilated, axis=0).astype(bool)
